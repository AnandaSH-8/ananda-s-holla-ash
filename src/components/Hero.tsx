      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center relative z-10">
        ...
        <div className="text-center lg:text-left animate-hero-content-entrance order-2 lg:order-1">
          ...
          >
            ...
            {/* Click ripple effect */}
            {nameClicked && (
              <div className="absolute inset-0 bg-[#FF6B00]/20 rounded-2xl animate-ripple-effect"></div>
            )}
            
            <h1 className={`text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-black text-[#333] dark:text-[#EDEDED] mb-6 leading-tight relative z-10 transition-all duration-300 group-hover:scale-105 ${
              nameClicked ? 'animate-name-bounce text-[#FF6B00]' : 'animate-hero-glow'
            }`}>
                ...
                {"Ananda S Holla".split("").map((letter, index) => (
                  <span
                    ...
                    style={{ 
                      ...
                      filter: nameClicked ? 'drop-shadow(0 0 15px rgba(255, 107, 0, 0.8))' : 'none'
                    }}
                  >
                    {letter === " " ? "\u00A0" : letter}
                  </span>
            ...
            {nameClicked && (
              <>
                <div className="absolute top-4 right-4 w-3 h-3 bg-[#FFA500] rounded-full animate-sparkle-burst-1"></div>
                <div className="absolute bottom-4 left-4 w-2 h-2 bg-[#FF6B00] rounded-full animate-sparkle-burst-2"></div>
        ...
        <div className="flex justify-center items-center animate-hero-photo order-1 lg:order-2">
          <div className="relative w-96 h-96 lg:w-[500px] lg:h-[500px] xl:w-[600px] xl:h-[600px] bg-gradient-to-br from-[#FFF4E6] to-[#FF6B00]/20 dark:from-[#FF6B00]/10 dark:to-[#FFA500]/30 rounded-3xl flex items-center justify-center border-4 border-[#FF6B00] shadow-2xl hover-scale group overflow-hidden">
            ...
            <div className="absolute inset-0 rounded-3xl border-4 border-[#FF6B00] animate-glow-border opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
